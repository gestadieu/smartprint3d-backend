...
app.get('/api', (request, response) => {
  ...
  if (search) {
    find['$where'] = function () {
      const obj = this;
      const keys = ['name', 'year', 'place', 'country', 'typeofplace', 'type', 'interactivity'];
      let result = false;
      keys.forEach(elt => {
  ...
  db.find(find).sort(sort).skip(skip).limit(limit).exec((err, docs) => {
    if (err) {
      response.end();
      return;
    }
...
app.post('/new', (request, response) => {
  ...
  db.insert(data, (err, doc) => {
    if (!err)
      response.redirect(`/show/${doc._id}`);
  });
});
